        ...
        <main class="flex-1 overflow-y-auto">
            ...
            <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex justify-between items-center mb-6">
                    <Link :href="route('medical-records.show', patient.id)" class="text-indigo-600 hover:text-indigo-800 font-medium flex items-center transition bg-white px-4 py-2 rounded-lg shadow-sm border border-gray-200">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                        Volver al Expediente
                    </Link>
                    
                    <button class="bg-gray-800 text-white px-4 py-2 rounded-lg text-sm font-bold shadow-md hover:bg-gray-700 transition flex items-center">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
                        Imprimir / PDF
                    </button>
                </div>

                <div class="bg-white shadow-2xl rounded-sm border border-gray-200 overflow-hidden print:shadow-none print:border-none">
                    
                    <div class="border-b-4 border-indigo-900 bg-gray-50 px-8 py-6 flex flex-col sm:flex-row justify-between items-start sm:items-center">
                        <div>
                            <h1 class="text-2xl font-black text-gray-900 uppercase tracking-tight">Nota Evolutiva</h1>
                            <p class="text-sm text-gray-500 font-medium mt-1">Expediente: {{ patient.clinics?.[0]?.pivot?.expediente_fisico || 'N/A' }}</p>
                        </div>
                    ...
                    <div class="px-8 py-5 border-b border-gray-100 bg-white grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div>
                            <span class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Paciente</span>
                            <span class="block text-sm font-bold text-gray-900">{{ patient.nombre }}</span>
                        </div>
                        <div>
                            <span class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Edad</span>
                            <span class="block text-sm font-medium text-gray-800">{{ edad }} años</span>
                        </div>
                        <div>
                            <span class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Sexo</span>
                            <span class="block text-sm font-medium text-gray-800">{{ patient.sexo || 'N/A' }}</span>
                        </div>
                        <div>
                            <span class="block text-xs font-bold text-red-400 uppercase tracking-wider mb-1">Alergias</span>
                            <span class="block text-sm font-bold text-red-600 truncate" :title="patient.medical_history?.alergias || 'Negadas'">
                    ...
                    <div class="px-8 py-8 space-y-8 bg-white">
                        ...
                        <div>
                            <h3 class="text-sm font-bold text-indigo-900 border-b border-gray-200 pb-2 mb-3 uppercase tracking-widest">S. Motivo de Consulta (Subjetivo)</h3>
                            <p class="text-gray-800 text-sm leading-relaxed whitespace-pre-line">{{ consultation.motivo_consulta }}</p>
                        </div>

                        <div v-if="consultation.exploracion_fisica">
                            <h3 class="text-sm font-bold text-indigo-900 border-b border-gray-200 pb-2 mb-3 uppercase tracking-widest">O. Exploración Física (Objetivo)</h3>
                            <p class="text-gray-800 text-sm leading-relaxed whitespace-pre-line">{{ consultation.exploracion_fisica }}</p>
                        </div>

                        <div v-if="consultation.diagnostico">
                            <h3 class="text-sm font-bold text-indigo-900 border-b border-gray-200 pb-2 mb-3 uppercase tracking-widest">A. Análisis / Diagnóstico</h3>
                            <p class="text-gray-800 text-sm leading-relaxed whitespace-pre-line">{{ consultation.diagnostico }}</p>
                        </div>

                        <div v-if="consultation.tratamiento" class="bg-indigo-50 p-6 rounded-xl border border-indigo-100">
                            <h3 class="text-sm font-bold text-indigo-900 mb-3 uppercase tracking-widest flex items-center">
                                <svg class="w-5 h-5 mr-2 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"></path></svg>
                                P. Plan y Tratamiento Médico
                            </h3>
                            <p class="text-indigo-900 text-sm leading-relaxed whitespace-pre-line font-medium">{{ consultation.tratamiento }}</p>
                        </div>
                    ...
                    <div class="px-8 py-8 bg-gray-50 border-t border-gray-200 text-center mt-12">
                        ...
                        <p class="text-xs text-gray-500 mt-1">Documento inmutable generado bajo la NOM-024 del Expediente Clínico Electrónico.</p>
                        <p class="text-xs text-gray-400 mt-1 font-mono">ID: {{ consultation.id }}-{{ new Date(consultation.created_at).getTime() }}</p>
                    </div>
